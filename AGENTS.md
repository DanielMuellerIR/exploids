...
"Der Götze" head boss and the space-cat minibosses), a sweeping laser
...
- **v0.10.0-Playtest abnehmen + Balance-Tuning**: Daniels Playtest zu HEAD `0031c71` auswerten — Balance-Gefühl prüfen (weniger Objekte, schwarze Löcher 20 % kleiner + seltener, erst ab Level 5; Power-up-Sammelradius 30→40), Mündungsblitz + Treffer-Feedback der Katze, Kopf-Look (Zardoz-Textur, bewegliche Augen). Ggf. Tuning-Konstanten justieren: `SpaceCat` (`artHeight`/`collisionRadius`/`bodyCenterNorm`/`eyeNorm`), `FloatingHead` (`headHeight`/`leftEyeNorm`/`rightEyeNorm`/`mouthNorm`), `GravityWell`-Radien/Stärke. (ex-theplan #201)
- **v0.11.1 veröffentlichen**: `git push` (2 ausstehende Commits) + `bash wrappers/sign-and-release.sh --publish` (Tag `v0.11.1` + signiertes DMG + GitHub-Release). Enthält den autoFire-Replay-Fix. Danach Angebot: Daniel spielt eine frische Runde → GIF aus demselben Binary rendern. (ex-theplan #204)
- **Fixed-Timestep-Mikro-Ruckler beobachten**: Daniel meldete in v0.12.0 einen möglichen Mikro-Ruckler (unsicher, evtl. Fremdlast auf dem Rechner). Falls reproduzierbar: `GameScene.simStepsPerSecond` von 120 auf 240 erhöhen (Einzeiler — feinerer Schritt, robuster gegen Takt-Jitter). (ex-theplan #221)
- **v0.12.1-Änderungen** (Replay-Szenengrössen-Fix + `--render-video mp4`): mit dem v0.13.0-Release nach GitHub veröffentlicht und notarisiert. Bestehende Replays rendern bereits korrekt; das Release sorgt dafür, dass der installierte App-Build künftig die Szenengrösse direkt mitaufnimmt. (ex-theplan #223)
...
- **Promo-GIF aus v0.12.0-Lauf**: Daniel liefert einen guten Replay-Lauf nach (neue notarisierte v0.12.0 spielen; jeder Game Over landet als Datei in `~/Library/Application Support/Exploids/replays`), dann GIF rendern via `exploids --render-last-replay --out <gif>` oder gezielt die Datei mit dem höchsten Score. (ex-theplan #220)
- **Finales Promo-GIF aus dem 76710-Replay**: Replay-Datei `~/Library/Application Support/Exploids/replays/2026-06-25_191055_score-76710_lvl-10.replay`; Review-Video `~/Desktop/exploids-replay-76710.mp4` (Länge 8:01, Echtzeit). Sobald Daniel Start-Zeitpunkt + Dauer aus dem Review-Video nennt: `exploids --render-replay <replay> --out x.gif --from <sekunden*120> --max-frames <dauer_s*30>` (HUD aus per Default, Zielraum 6–10 s, max ~15 s). (ex-theplan #222)
...
**Nächste geplante Phase:** Nach Abschluss des v0.10.0-Playtests (siehe Open To-Dos) deterministisches Replay-System Phase 1 fortsetzen: `GameRandom` PRNG + Determinismus-Regressionstest (Details in `docs/replay-system-plan.md`). Trace-Pipeline `/tmp/exploids-trace` (potrace/rsvg/imagemagick), Quellbilder `~/Desktop/katze.png` + `kopf.webp`; Hinweis: `magick`-SVG-Render IMMER `-background none` VOR die Input-SVG stellen (sonst weisser Hintergrund). (ex-theplan #201)
